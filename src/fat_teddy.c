...
void
pattern_mask_add_fat (struct PatternMask *mask, char byte, uint8_t bucket_id)
{
...
void
pattern_mask_finish (struct PatternMask *mask)
{
...
void
pattern_mask_init (struct PatternMask *pattern_mask, struct Bucket *buckets, char **patterns)
{
...
void
fat_teddy_init (struct FatTeddy *teddy, char **patterns, uint8_t num_patterns)
{
...
__m256i
lookup_1 (__m256i *chunk, struct PatternMask *pattern_mask)
{
...
struct Match
fat_teddy_verify64 (struct FatTeddy *teddy, uint64_t lane, char *str, size_t str_size)
{
        while (lane != 0)
        {
...
struct Match
fat_teddy_verify (struct FatTeddy *teddy, __m256i *candidate, char *str, size_t str_size)
{
        ...
        struct Match match = fat_teddy_verify64 (teddy, lane, str, str_size);
...
struct Match
fat_teddy_find_next (struct FatTeddy *teddy, char *str, size_t str_size)
{
...
struct Match
fat_teddy_find (struct FatTeddy *teddy, char *str, const size_t str_size)
{
        assert (str_size >= 16);
        ...
        while (size >= 16)
        {
                struct Match match = fat_teddy_find_next (teddy, start, size);
                if (match.pattern_id >= 0)
                {
                        return match;
                }
